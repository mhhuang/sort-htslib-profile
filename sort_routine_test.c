...
// write tmp files, always use 1 thread for each file
void write_tmp_files(const char* name, const char* mode, int size, bam1_p * buffer,
    const bam_hdr_t *header) {

  int i;
  samFile* sf = sam_open_format(name, mode, NULL);
  if (sf == NULL) return;
  sam_hdr_write(sf, header);
  //if (n_threads > 1) hts_set_threads(sf, n_threads);
  //((fp->fp).bgzf)->compress_level = (signed)0;
  for (i = 0; i < size; ++i)
    sam_write1(sf, header, buffer[i]);
  sam_close(sf);
...
static void *worker(void *data) {
  ...
  //pid_t this_id = syscall(SYS_gettid);
  ...
  write_tmp_files(name, w->modeout, w->buf_len, w->buf, w->header);

  free(name);
  return 0;
}

int write_blocks(int nof_files, int size, bam1_t** buffer, const bam_hdr_t *header,
    int nof_threads, const char *prefix, int compress_level) {
  ...
  if (nof_threads < 1) nof_threads = 1;
  // use a single thread if we only sort a small batch of records
  if (size < nof_threads * 64) nof_threads = 1;

  char modeout[4];
  //sam_open_mode(modeout+1, output_file, NULL);
  strcpy(modeout, "wb");
  if (compress_level > 0) {
    sprintf(modeout+2, "%d", compress_level < 9 ? compress_level : 9);
  }

  pthread_attr_init(&attr);
  pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
  w = (worker_t*)calloc(nof_threads, sizeof(worker_t));
  tid = (pthread_t*)calloc(nof_threads, sizeof(pthread_t));
  bam1_p *b = buffer;
  int rest = size;
  for (i = 0; i < nof_threads; ++i) {
    w[i].buf_len = rest / (nof_threads - i);
    w[i].buf = b;
    w[i].prefix = prefix;
    w[i].header = header;
    w[i].index = nof_files + i;
    w[i].modeout = modeout;
    b += w[i].buf_len;
    rest -= w[i].buf_len;
    pthread_create(&tid[i], &attr, worker, &w[i]);
  }
  for (i = 0; i < nof_threads; ++i) pthread_join(tid[i], 0);
  free(tid); free(w);

  gettimeofday(&t_end, NULL);
  fprintf(stdout, "%d parallel writes:  time for the last 1000000 records, %ld us\n",
      nof_threads, getTimeDiffInUs(t_start, t_end));
  return nof_threads;
}

int sort_routine_test(char* input_file, char* output_file,
    int nof_threads, int compress_level, char* tmp_prefix) {
  ...
  for(i = 0;;) {
    ...
    //if (i < 3000000) continue; // continue to read
    ...
    // TODO(mhhuang) can Test randome reads 
    {
      ...
      fns = (char**)calloc(nof_files, sizeof(char*));
      int j;
      for (j = 0; j < nof_files; ++j) {
        fns[j] = (char*)calloc(strlen(tmp_prefix) + 20, 1);
        sprintf(fns[j], "%s.%.4d.bam", tmp_prefix, j);
      }
      //return 0;
...
void add_elapsed_time(uint64_t *time, struct timespec start, struct timespec end) {
...
// TODO(mhhuang): test only
//    hts_set_threads(fp[i], 4);
// End of test
        if (fp[i] == NULL) {
            int j;
    ...
    uint64_t heapfy_time = 0, read_time = 0, write_time = 0;
    uint64_t elapsed_time = 0;
    struct timespec all_start, all_end;
    struct timespec start, end;
    uint64_t nof_records = 0;
clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &all_start);
    // Begin the actual merge
//clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
    ks_heapmake(heap, n, heap);
//clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);
//add_elapsed_time(&heapfy_time, start, end);
...
//clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);
//add_elapsed_time(&read_time, start, end);
//clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
        sam_write1(fpout, hout, b);
        nof_records++;
//clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);
//add_elapsed_time(&write_time, start, end);
//clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
...
//fprintf(stdout, "read_time:   %f us/rec\n", read_time/1000.0/nof_records);
//fprintf(stdout, "heapfy_time: %f us/rec\n", heapfy_time/1000.0/nof_records);
//fprintf(stdout, "write_time:  %f us/rec\n", write_time/1000.0/nof_records);
clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &all_end);
add_elapsed_time(&elapsed_time, all_start, all_end);
fprintf(stdout, "ovreall_time:  %f us/rec\n", elapsed_time/1000.0/nof_records);
